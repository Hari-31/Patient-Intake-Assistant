...
- Do not render summary controls until `intake_complete` is true. Emergency
  escalation is the only exception.
...
- When `emergency_triggered` is true, display the backend reply prominently and
  stop further messages in that intake. A patient may explicitly start a new
  intake afterward.
- Use a synchronous in-flight guard in addition to disabled controls so rapid
  duplicate submit events cannot create duplicate backend messages.
...
- Refresh near-expiry Supabase sessions before backend calls. A backend `401`
  signs the user out locally and redirects to login.
- Normalize `403` and `404` responses to the same neutral not-found message so
  the UI does not reveal whether a resource exists.
...
1. Should the first screen after login be a new patient intake, a previous intake
   list, or a role-based dashboard?
2. Should patients be able to resume old sessions across browsers and devices?
3. Should patients be able to edit a generated summary before sharing it?
4. Are uploaded reports limited to labs, imaging reports, discharge notes, or any
   medical PDF?
5. Is this intended for education, clinic workflow support, or production care
   coordination?
